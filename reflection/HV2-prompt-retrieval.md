...
|---|---|---|
| Định nghĩa logic suy luận mức hiểu (4 tín hiệu) | `codebase/src/lib/inference.ts` & Canvas CP1 | CP1 (19:30 17/9) |
| Định nghĩa logic phát hiện segment hỏng | `codebase/src/lib/segment-detect.ts` & Canvas CP1 | CP1 (19:30 17/9) |
| Mock pipeline (TypeScript, chạy không cần API key) | `codebase/src/lib/gemini-mock.ts` | CP2 (21:00 17/9) |
| Pipeline thật + gọi Gemini API + video 30s | `codebase/src/lib/gemini.ts` + video demo | CP3 (16:00 18/9) |
| Golden Set 20 cases + runner + đo Quality Bar | `codebase/tests/hv2-questions.ts`, `codebase/tests/hv2-run.ts`, `codebase/tests/HV2-RESULTS.md` | CP4 (21:00 18/9) |
...
Toàn bộ backend pipeline được viết bằng **TypeScript** tích hợp trực tiếp trong thư mục `codebase/src/lib/`, giúp Web App chạy độc lập, tốc độ xử lý $< 1\text{ms}$ cho các khâu tiền xử lý (pre-processing), không cần dựng thêm server Python riêng lẻ:
...
└───────────────┼────────────────────────────────────────┼────────────────────┘
                ...
                ▼                                        │
┌─────────────────────────────────────────────────────────────────────────────┐
│                  HV2 AI Pipeline (codebase/src/lib/)                            │
│                                                                             │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ 1. SEGMENT DETECTION (codebase/src/lib/segment-detect.ts)                 │  │
...
│                                     ▼                                        │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ 2. UNDERSTANDING INFERENCE (codebase/src/lib/inference.ts)                │  │
...
│                                     ▼                                        │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ 3. PROMPT & GENERATION (codebase/src/lib/gemini.ts)                       │  │
│  │    • Nếu không có API Key → fallback sang codebase/src/lib/gemini-mock.ts │  │
...
## 3. Chi tiết các File TypeScript cốt lõi trong `codebase/src/lib/`

### 3.1. Thuật toán suy luận mức hiểu: `codebase/src/lib/inference.ts`

File này chứa toàn bộ logic toán học và biểu thức Regex phân tích hành vi người học:

```typescript
// codebase/src/lib/inference.ts
...
### 3.2. Bộ lọc phát hiện Segment hỏng: `codebase/src/lib/segment-detect.ts`
...
```typescript
// codebase/src/lib/segment-detect.ts
...
### 3.3. Tích hợp Gemini API & Prompt Tự Nhiên: `codebase/src/lib/gemini.ts`
...
### 4.1. Bộ 20 câu hỏi Golden Set: `codebase/tests/hv2-questions.ts`
...
### 4.2. Test Runner: `codebase/tests/hv2-run.ts`
Chạy kiểm thử trực tiếp qua dòng lệnh:
```bash
npx tsx tests/hv2-run.ts
```

### 4.3. Kết quả đo đạc thực tế: `codebase/tests/HV2-RESULTS.md`
